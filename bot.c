    ...
    for(int i = 0; i < usedEntries; i++){
        ...
        while(current != NULL){
            if(current->used == 0 && strcmp(current->name, optionsSorted[i].word) != 0){ //second condition because we cannot use the same word twice
                ...
                if(difficulty > 1){
                    if(strcmp("RESIGN",playBot(spellMap, tally, current->name, difficulty-1)) == 0){ //if i am forced to resign when opponent plays this word, then i should not alow him to play it
                        canLose = 1;
                        break;
                    }
                }
